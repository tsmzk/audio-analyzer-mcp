...
# ------------------------------------------------------------------
# ピッチ推定(librosa.pyin)の探索範囲
# ------------------------------------------------------------------
# C2 ≈ 65Hz(男性低音の下限), C7 ≈ 2093Hz(人の声の高音の上限)。
# 実際の fmax はサンプリングレートの Nyquist 周波数 (sr/2) を超えられないので、
# analyzer 側でさらに下げてから pyin に渡す(ハードコードは安全な上限値)。
PITCH_FMIN_HZ = 65.41          # C2
PITCH_FMAX_HZ = 2093.00        # C7

# サンプリングレートと fmax の関係:
#   fmax <= sr / 2 (Nyquist) が必要。
#   pyin 内部で細かい判定があるため安全率を掛けて fmax <= sr / NYQUIST_SAFETY を採用する。
NYQUIST_SAFETY = 2.1
...
# 長尺動画の自動分割処理
# ------------------------------------------------------------------
# MCP のツール呼び出しは応答が長引くとクライアント側でタイムアウトしやすい。
# また librosa.pyin は長すぎる入力に対して内部バリデーション
# (`Target size ... must be at least input size ...`)で落ちることがある。
# そこで一定時間を超える音声はチャンクに分けて順次処理し、最後に結合する。
#
# 分割時のポイント:
#   - 各チャンクは独立に librosa.pyin にかける(内部制約を回避)
#   - 前後にオーバーラップを入れ、境界付近の検出漏れ & スパイク判定の warmup に使う
#   - rms_norm(音量正規化)は全チャンク処理後に全体の分布で再計算

AUTO_CHUNK_THRESHOLD_SEC = 20 * 60  # これを超える音声はチャンク分割(秒)
CHUNK_DURATION_SEC = 15 * 60        # 1 チャンクの長さ(秒)
CHUNK_OVERLAP_SEC = 15              # チャンク境界のオーバーラップ(秒)

# ------------------------------------------------------------------
# 並列処理設定
# ------------------------------------------------------------------
# チャンク解析を別プロセスで並走させる時の最大ワーカー数。
# 各チャンクの pyin/RMS/重心抽出は完全に独立しているため、
# CPU コア数ぶん並列化することで長尺ファイルの解析時間を
# おおよそリニアに短縮できる(3時間ファイルで 4分タイムアウトに収めるための主要因)。
#
# - 0 以下を指定したら os.cpu_count() に合わせて自動選択(上限 PARALLEL_WORKERS_CAP)
# - 1 にすると並列化を無効化(逐次処理にフォールバック)
PARALLEL_WORKERS = 0
# プロセス数の上限。各ワーカーは ~80MB のメモリを使うので、過剰に並列化しても
# メモリ・スレッド競合で逆効果。経験則的に 8 を上限とする。
PARALLEL_WORKERS_CAP = 8
